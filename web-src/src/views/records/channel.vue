...
const onDay = (y, m, d) => {
  router.push({
    params: { date: `${dayjs(`${y}-${m}-${d}`).format('YYYYMMDD')}` },
  })
}
const onDate = (y, m) => {
  router.push({
    params: {
      date: `${dayjs(`${y}-${m}-${dateDay.value}`).format('YYYYMMDD')}`,
    },
  })
}
const initDate = () => {
  ...
  if (Number.isNaN(v.$y) || Number.isNaN(v.$M) || Number.isNaN(v.$D)) {
    ...
    router.push({
      params: {
        date: `${dayjs(`${v.$y}-${v.$M + 1}-${v.$D}`).format('YYYYMMDD')}`,
      },
    })
  } else {
    dateYear.value = v.$y
...
const queryData = () => {
  const start = dayjs(route.params.date).startOf('day') // Start of the day
  const end = dayjs(route.params.date).endOf('day') // End of day
  records
    .getRecordsCloudInfo(route.params.id, {
      start_ms: start.valueOf(),
      end_ms: end.valueOf(),
    })
    .then(res => {
      if (res.status == 200) {
...
const queryDataMonth = () => {
  records
    .getRecordsCloudMonth(route.params.id, {
      dates: dayjs(route.params.date).format('YYYYMM'),
    })
    .then(res => {
      if (res.status == 200) {
    ...
    >
      <template #bodyCell="{ column, text, record }">
        ...
        <template v-if="column.key === 'duration'">
          <a-button type="info"> {{ parseInt(record.duration) }}秒 </a-button>
        </template>
