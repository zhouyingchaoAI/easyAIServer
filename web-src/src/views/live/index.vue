...
const columns = [
    ...
    { title: '地址', align: 'center', width: 320, dataIndex: 'url', key: 'url' },
...
const copyUrl = (key,id) => {
    let url = ""
    let idStr = `stream_${id}`
    switch (key) {
    case "http_flv":
        url = `${location.origin}/live/${idStr}.flv`
        break;
    case "http_hls":
        url = `${location.origin}/hls/ts/${idStr}/ts-playlist.m3u8`
        break;
    case "ws_flv":
        url = `ws://${location.host}/live/${idStr}.flv`
        break;
    case "webrtc":
        url = `webrtc://${location.host}/webrtc/play/live/${idStr}.whep`
        break;
    case "rtmp":
        url = `rtmp://${location.hostname}${objPort.value.rtmp}/live/${idStr}`
        break;
    case "rtsp":
        url = `rtsp://${location.hostname}${objPort.value.rtsp}/live/${idStr}`
        break;
    default:
        break;
    }
        ...
        <a-table :columns="columns" :data-source="dataList" :pagination="false" :scroll="{ x: 1200 }">
            <template #bodyCell="{ column, text, record }">
                ...
                <template v-if="column.key === 'url'">
                   <span class="cursor-p">
                    <!-- http_flv http_hls ws_flv webrtc rtmp rtsp -->
                        <a-tag class="mr2px" @click="copyUrl('webrtc',record.id)" color="default" >WebRTC<CopyOutlined class="ml2px"/></a-tag>
                        <a-tag class="mr2px" @click="copyUrl('http_hls',record.id)" color="default" >HLS<CopyOutlined class="ml2px"/></a-tag>
                        <a-tag class="mr2px" @click="copyUrl('http_flv',record.id)" color="default" >FLV<CopyOutlined class="ml2px"/></a-tag>
                        <a-tag class="mr2px" @click="copyUrl('ws_flv',record.id)" color="default" >WS_FLV<CopyOutlined class="ml2px"/></a-tag>
                        <a-tag class="mr2px" @click="copyUrl('rtsp',record.id)" color="default" >RTSP<CopyOutlined class="ml2px"/></a-tag>
                   </span>
                </template>
